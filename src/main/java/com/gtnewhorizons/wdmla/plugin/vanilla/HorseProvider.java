    ...
    public void appendTooltip(ITooltip tooltip, EntityAccessor accessor) {
        if(accessor.getEntity() instanceof EntityHorse horse) {
            ...
            tooltip.child(ThemeHelper.INSTANCE.value(
                    ...
                    String.format("%.2f", getJumpHeight(jumpStrength)) + StringUtils.SPACE + LangUtil.translateG("hud.msg.wdmla.blocks")
            ));
            ...
            tooltip.child(ThemeHelper.INSTANCE.value(
                    ...
                    String.format("%.2f", movementSpeed * SPEED_UNIT_TO_BLOCKS_PER_SECOND_RATE) + StringUtils.SPACE + LangUtil.translateG("hud.msg.wdmla.blockspersecond")
            ));
            if(accessor.showDetails()) {
