...
public class GenerateDumpCommand extends GTNHClientCommand {
    ...
    public void processCommand(ICommandSender sender, String[] args) {
        ...
        try (FileWriter writer = new FileWriter(file)) {
            ...
            GTNHLib.proxy.addMessageToChat(new ChatComponentText(
                    String.format(LangUtil.translateG("chat.msg.wdmla.dump.success"), file.getCanonicalPath())));
        } catch (IOException e) {
    ...
    private static void createSection(StringBuilder builder, String subsection, IHierarchyLookup<? extends IWDMlaProvider> lookup) {
        ...
        lookup.entries().forEach(entry -> {
            ...
            entry.getValue()
                    ...
                    .forEach($ -> {
                        builder.append("\n* ").append($.getUid()).append(", ").append(WDMlaCommonRegistration.instance().priorities.byValue(
