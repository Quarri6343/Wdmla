...
import mcp.mobius.wdmla.util.McUtil;
...
public class OverlayRenderer {
    ...
    public static void renderOverlay(IHUDWidget root) {
        ...
        computePositionAndSize(root);
        renderElements(root);
    }

    public static void renderElements(IHUDWidget root) {
        GL11.glPushMatrix();
        saveGLState();
    ...
    public static void drawTooltipBox(int x, int y, int w, int h, int bg, int grad1, int grad2) {
        RenderUtil.drawGradientRect(x + 1, y, w - 1, 1, bg, bg);
        RenderUtil.drawGradientRect(x + 1, y + h, w - 1, 1, bg, bg);
        RenderUtil.drawGradientRect(x + 1, y + 1, w - 1, h - 1, bg, bg);// center
        RenderUtil.drawGradientRect(x, y + 1, 1, h - 1, bg, bg);
        RenderUtil.drawGradientRect(x + w, y + 1, 1, h - 1, bg, bg);
        RenderUtil.drawGradientRect(x + 1, y + 2, 1, h - 3, grad1, grad2);
        RenderUtil.drawGradientRect(x + w - 1, y + 2, 1, h - 3, grad1, grad2);

        RenderUtil.drawGradientRect(x + 1, y + 1, w - 1, 1, grad1, grad1);
        RenderUtil.drawGradientRect(x + 1, y + h - 1, w - 1, 1, grad2, grad2);
    }

    private static void computePositionAndSize(IHUDWidget root) {
        Point pos = new Point(
        ...
        w = root.getWidth() + MARGIN * 2;
        h = root.getHeight() + MARGIN * 2;

        Dimension size = McUtil.displaySize();
        x = ((int) (size.width / OverlayConfig.scale) - w - 1) * pos.x / 10000;
        y = ((int) (size.height / OverlayConfig.scale) - h - 1) * pos.y / 10000;

        ty = (h - root.getHeight()) / 2 + 1;
    }
